...
public class UserRegisteredEmailHandler : INonBlockingDomainEventHandler<UserRegisteredEvent>
{
    ...
    public async Task HandleAsync(UserRegisteredEvent domainEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var verificationUrl = $"https://localhost:7212/Identity/Account/VerifyEmail?email={Uri.EscapeDataString(domainEvent.Email)}&token={Uri.EscapeDataString(token)}"; await _emailService.SendEmailAsync(
                            domainEvent.Email,
                            "Verify Your Email",
